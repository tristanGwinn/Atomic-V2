...
atomic::TrackingWheel::TrackingWheel(pros::adi::Encoder* encoder, float wheelDiameter, float distance,
                                     float gearRatio) {
...
atomic::TrackingWheel::TrackingWheel(pros::Rotation* encoder, float wheelDiameter, float distance, float gearRatio) {
...
atomic::TrackingWheel::TrackingWheel(pros::MotorGroup* motors, float wheelDiameter, float distance, float rpm) {
    this->motors = motors;
...
void atomic::TrackingWheel::reset() {
...
float atomic::TrackingWheel::getDistanceTraveled() {
    if (this->encoder != nullptr) {
        ...
        return atomic::avg(distances);
    } else {
        return 0;
    }
}

float atomic::TrackingWheel::getOffset() { return this->distance; }

int atomic::TrackingWheel::getType() {
