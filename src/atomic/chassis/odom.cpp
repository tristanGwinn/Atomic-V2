...
// global variables
atomic::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr, nullptr); // the sensors to be used for odometry
atomic::Drivetrain drive(nullptr, nullptr, 0, 0, 0, 0); // the drivetrain to be used for odometry
atomic::Pose odomPose(0, 0, 0); // the pose of the robot
atomic::Pose odomSpeed(0, 0, 0); // the speed of the robot
atomic::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void atomic::setSensors(atomic::OdomSensors sensors, atomic::Drivetrain drivetrain) {
    odomSensors = sensors;
    drive = drivetrain;
}

atomic::Pose atomic::getPose(bool radians) {
    if (radians) return odomPose;
    else return atomic::Pose(odomPose.x, odomPose.y, rad_to_deg(odomPose.theta));
}

void atomic::setPose(atomic::Pose pose, bool radians) {
    if (radians) odomPose = pose;
    else odomPose = atomic::Pose(pose.x, pose.y, deg_to_rad(pose.theta));
}

atomic::Pose atomic::getSpeed(bool radians) {
    if (radians) return odomSpeed;
    else return atomic::Pose(odomSpeed.x, odomSpeed.y, rad_to_deg(odomSpeed.theta));
}

atomic::Pose atomic::getLocalSpeed(bool radians) {
    if (radians) return odomLocalSpeed;
    else return atomic::Pose(odomLocalSpeed.x, odomLocalSpeed.y, rad_to_deg(odomLocalSpeed.theta));
}

atomic::Pose atomic::estimatePose(float time, bool radians) {
...
void atomic::update() {
    ...
    if (odomSensors.imu != nullptr) imuRaw = deg_to_rad(odomSensors.imu->get_rotation());
    ...
    atomic::TrackingWheel* verticalWheel = nullptr;
    atomic::TrackingWheel* horizontalWheel = nullptr;
