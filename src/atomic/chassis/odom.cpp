...
atomic::Odometry::Odometry(std::vector<IMU*> imus, std::vector<TrackingWheel*> verticalWheels,
                                             std::vector<TrackingWheel*> horizontalWheels)
    : m_Imus(imus),
      m_verticalWheels(verticalWheels),
      m_horizontalWheels(horizontalWheels) {}

units::Pose Odometry::getPose() { return m_pose; }

void Odometry::setPose(units::Pose pose) {
    m_offset += pose.orientation - m_pose.orientation;
    m_pose = pose;
}

void Odometry::startTask(Time period) {
 ...
 */
void Odometry::update(Time period) {
