 ...
 * that represent pose, time elapsed, linear velocity, and angular velocity
 * at a point.
 */
class Trajectory {
    public:
        ...
        struct State {
            ...
            LinearVelocity linearVelocity = 0_mps;

            // state angular velocity
            AngularVelocity angularVelocity = 0_rps;
            ...
            State(Time ta, LinearVelocity linearV, AngularVelocity angularV, units::Pose p) :
              t(ta), linearVelocity(linearV), angularVelocity(angularV), pose(p)
            {}
         ...
         */
        units::Pose initialPose() const { return sample(0_sec).pose; }
